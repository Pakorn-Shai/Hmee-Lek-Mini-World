...
function createNode(cc, name, parent) {
...
function getOrCreateChild(cc, parent, name) {
...
async function queryAssetInfo(url) {
    try {
        ...
        warn(`Asset query failed for ${url}: ${error.message}`);
        return null;
    }
}
function extractSpriteFrameUuid(assetInfo) {
    const subAssets = assetInfo?.subAssets;
    if (Array.isArray(subAssets)) {
        const spriteFrame = subAssets.find((assetInfoItem) => assetInfoItem.type === 'cc.SpriteFrame' || assetInfoItem.name === 'spriteFrame');
        return spriteFrame?.uuid ?? null;
    }
    if (subAssets && typeof subAssets === 'object') {
        for (const subAsset of Object.values(subAssets)) {
            if (subAsset?.type === 'cc.SpriteFrame' ||
...
async function querySpriteFrameAsset(spec) {
...
async function findSpriteFrameUuid(spec, report) {
...
async function loadSpriteFrame(cc, uuid) {
    return await new Promise((resolve) => {
        cc.assetManager.loadAny({ uuid }, (error, loadedAsset) => {
            if (error) {
...
async function setSpriteFrame(cc, sprite, spriteInfo) {
    if (!spriteInfo) {
...
function findFirstCameraInTree(node, cameraType) {
...
function buildSceneCamera(cc, scene, canvasNode) {
    const canvasCamera = canvasNode.children
        .find((child) => child.name === 'Camera')
...
function getDesignResolution(cc) {
...
async function buildBackground(cc, canvasNode, spriteInfo) {
...
function buildSafeArea(cc, canvasNode) {
...
function buildPageTitle(cc, safeArea) {
...
function buildStageGrid(cc, safeArea) {
...
async function buildStageButton(cc, parent, index, spriteInfo, interactable) {
...
async function buildBackButton(cc, safeArea, spriteInfo) {
...
async function buildBubbleStageSelect() {
    ...
    const assetReport = { found: [], missing: [] };
    ...
    for (let index = 1; index <= 10; index += 1) {
        await buildStageButton(cc, stageGrid, index, index === 1 ? unlockedStageButton : lockedStageButton, index === 1);
    }
